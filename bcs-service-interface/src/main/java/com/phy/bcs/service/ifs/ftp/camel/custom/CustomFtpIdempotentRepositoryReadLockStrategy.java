...
/**
 * A file read lock that uses an {@link IdempotentRepository} as the lock strategy. This allows to plugin and use existing
 ...
 */
public class CustomFtpIdempotentRepositoryReadLockStrategy extends ServiceSupport implements GenericFileExclusiveReadLockStrategy<FTPFile>, CamelContextAware {

    private GenericFileEndpoint<FTPFile> endpoint;
    ...
    @Override
    public void prepareOnStartup(GenericFileOperations<FTPFile> operations, GenericFileEndpoint<FTPFile> endpoint) throws Exception {
        this.endpoint = endpoint;
        //log.info("Using CustomFtpIdempotentRepositoryReadLockStrategy: {} on endpoint: {}", idempotentRepository, endpoint);
    }
    ...
    public void releaseExclusiveReadLockOnRollback(GenericFileOperations<FTPFile> operations, GenericFile<FTPFile> file, Exchange exchange) throws Exception {
        ...
        if (readLockIdempotentReleaseDelay > 0 && readLockIdempotentReleaseExecutorService != null) {
            ...
            readLockIdempotentReleaseExecutorService.schedule(r, readLockIdempotentReleaseDelay, TimeUnit.SECONDS);
    ...
    protected String asKey(GenericFile<FTPFile> file) {
        // use absolute file path as default key, but evaluate if an expression key was configured
        String key = "readLock:" + file.getAbsoluteFilePath();
