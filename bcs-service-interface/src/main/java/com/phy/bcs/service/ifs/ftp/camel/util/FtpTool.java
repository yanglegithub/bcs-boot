...
public class FtpTool {
    ...
    public boolean initFtpClient() {
        ...
        try {
            isready = ftpClient.sendNoOp();
        } catch (Exception e) {
            log.debug("connecttion ip:{} 连接中断", hostname);
            isready = false;
        }
        if(!isready) {
            if(ftpClient == null)
            ...
            try {
                log.debug("connecting...ftp服务器:" + this.hostname + ":" + this.port);
                ftpClient.connect(hostname, port); //连接ftp服务器
                ftpClient.login(username, password); //登录ftp服务器
                int replyCode = ftpClient.getReplyCode(); //是否成功登录服务器
                if (!FTPReply.isPositiveCompletion(replyCode)) {
                    log.debug("connect failed...ftp服务器:" + this.hostname + ":" + this.port);
                    return false;
                }
                log.debug("connect successfu...ftp服务器:" + this.hostname + ":" + this.port);
                return true;
            }catch (SocketException e) {
                log.debug("连接失败 ip:{}", hostname);
                return false;
            } catch (IOException e) {
                log.debug("连接失败 ip:{}", hostname);
                return false;
    ...
    public boolean changeWorkingDirectory(String directory) {
        ...
        try {
            ...
            if (flag) {
                ...
                log.debug("入文件夹" + directory + " 失败！开始创建文件夹");
            }
